...
variable        nlat equal 15           # size of box
variable        T    equal 300  # Temperature in K
variable        rho  equal 3.1652       # Lattice spacing in A
...
pair_style      eam/fs
pair_coeff      * * ../gammasT/w_eam3.fs W
...
fix NPT all npt temp $T $T $(100*dt) aniso 1. 1. $(1000*dt) fixedpoint 0. 0. 0.
...
variable C1        equal "v_eV2J/v_Myvol" # Convert Cb from energy to pressure units
variable C2        equal "v_Myvol/v_kbt"       # Factor for Cfl terms
variable Pa2GPa    equal 1e-9

print       "$T ${Myvol}" append vol.txt
...
variable s1 equal "-c_VIR[1]*v_bar2Pa"
variable s2 equal "-c_VIR[2]*v_bar2Pa"
variable s3 equal "-c_VIR[3]*v_bar2Pa"
variable s6 equal "-c_VIR[4]*v_bar2Pa"
variable s5 equal "-c_VIR[5]*v_bar2Pa"
variable s4 equal "-c_VIR[6]*v_bar2Pa"
variable press equal press


# Average of Born term and vector to store stress
# for post processing
fix CB all ave/time 1 ${nthermo} ${nthermo} c_born[*] ave running file born-$T.out overwrite
fix CPR all ave/time 1 1 1 c_VIR[*] file vir-$T.out
